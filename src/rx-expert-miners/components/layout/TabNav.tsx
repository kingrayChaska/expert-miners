...
const TABS: {
  ...
  { to: "/admin/dashboard", end: true, labelKey: "masterData", icon: Database },
  { to: "/admin/passing-by", labelKey: "passingBy", icon: UserRound },
  { to: "/admin/printing", labelKey: "printing", icon: Printer },
  { to: "/admin/data-bank", labelKey: "dataBank", icon: BarChart3 },
  { to: "/admin/settings", labelKey: "settings", icon: SettingsIcon, admin: true },
  { to: "/admin/users", labelKey: "users", icon: UsersIcon, admin: true },
];
      ...
      {TABS.filter((tab) => !tab.admin || canAdmin).map((tab) => {
        ...
        return (
          <NavLink
            ...
            className={({ isActive }) =>
              cn(
                "relative inline-flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-all duration-200",
