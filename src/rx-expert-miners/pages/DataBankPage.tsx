...
const DataBankPage = () => {
  ...
  useEffect(() => {
    ...
    const ch = (supabase as any)
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "master_data" },
        (p: any) => {
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "passing_by" },
        (p: any) => {
          if (flushTimer.current) clearTimeout(flushTimer.current);
  ...
  const stats = useMemo(() => {
    ...
    const dispatched = m.filter(
      (r) => r.dispatch_status === "Dispatch" || r.dispatch_date,
    ).length;
    const inRepair = m.filter(
      (r) => (r.repair_status || r.aging_status) && !r.dispatch_date,
    ).length;
    const awaiting = m.filter(
      (r) => r.client_approval === "Awaiting approved",
    ).length;
    const ready = m.filter(
      (r) => r.ready_for_dispatch === "Yes" && !r.dispatch_date,
    ).length;
    const warranty = m.filter((r) => r.warranty_status === "Warranty").length;
    const outWarranty = m.filter(
      (r) => r.warranty_status === "Out of 6",
    ).length;
    const scrapped = m.filter(
      (r) => (r.final_status || "").trim() === "Scrape",
    ).length;
    const uniqueClients = new Set(
      m.map((r) => (r.client_name || "").trim()).filter(Boolean),
    ).size;
    ...
    const serialsReady = pickSerials(
      m.filter((r) => r.ready_for_dispatch === "Yes" && !r.dispatch_date),
    );
    const serialsInRepair = pickSerials(
      m.filter((r) => (r.repair_status || r.aging_status) && !r.dispatch_date),
    );
    const serialsAwaiting = pickSerials(
      m.filter((r) => r.client_approval === "Awaiting approved"),
    );
    const serialsDispatched = pickSerials(
      m.filter((r) => r.dispatch_status === "Dispatch" || r.dispatch_date),
    );
    const serialsWarranty = pickSerials(
      m.filter((r) => r.warranty_status === "Warranty"),
    );
    const serialsOutWarranty = pickSerials(
      m.filter((r) => r.warranty_status === "Out of 6"),
    );
    const serialsScrapped = pickSerials(
      m.filter((r) => (r.final_status || "").trim() === "Scrape"),
    );
  ...
  const cards = [
    {
      ...
      tint: "primary",
      v: stats.total,
      l: label("Total Master Records", "إجمالي السجلات"),
    },
    {
      icon: PackageCheck,
      tint: "success",
      v: stats.dispatched,
      l: label("Dispatched", "تم التسليم"),
    },
    {
      icon: Wrench,
      tint: "warning",
      v: stats.inRepair,
      l: label("In Repair", "قيد الصيانة"),
    },
    {
      icon: Clock,
      tint: "warning",
      v: stats.awaiting,
      l: label("Awaiting Approval", "بانتظار الموافقة"),
    },
    {
      icon: CheckCircle2,
      tint: "success",
      v: stats.ready,
      l: label("Ready for Dispatch", "جاهز للتسليم"),
    },
    {
      icon: Users,
      tint: "primary",
      v: stats.uniqueClients,
      l: label("Unique Clients", "العملاء الفريدون"),
    },
    {
      icon: BarChart3,
      tint: "primary",
      v: stats.last30,
      l: label("Received (30d)", "مستلم آخر 30 يوم"),
    },
    {
      icon: UserRound,
      tint: "primary",
      v: stats.passingTotal,
      l: label("Passing By Entries", "سجلات العابرين"),
    },
    {
      icon: Trash2,
      tint: "destructive",
      v: stats.scrapped,
      l: label("Scrapped", "مهملة"),
    },
  ];
    ...
    <div dir={dir} className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold">
            {label("Data Bank", "بنك البيانات")}
          </h1>
          <p className="text-sm text-muted-foreground">
            {label(
              "Live statistics across all records",
              "إحصاءات حية لجميع السجلات",
            )}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-end gap-2 rounded-xl border border-border bg-card p-3 shadow-sm">
        ...
        <div className="flex flex-col gap-1 flex-1 min-w-[200px]">
          <label className="text-[10px] uppercase text-muted-foreground">
            {label("Serial Number", "الرقم المتسلسل")}
          </label>
          <Input
            value={serialFilter}
            onChange={(e) => setSerialFilter(e.target.value)}
            placeholder={label(
              "Search serial / PSU / hashboard…",
              "ابحث بالرقم المتسلسل…",
            )}
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {cards.map((c) => {
              ...
              return (
                <div
                  key={c.l}
                  className="rounded-2xl border border-border bg-card p-4 flex items-start gap-3 shadow-sm"
                >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="rounded-2xl border border-border bg-card p-5 shadow-sm md:col-span-2">
              <h3 className="font-semibold mb-3">
                {label("Records by Month", "السجلات حسب الشهر")}
              </h3>
              {stats.byMonth.length === 0 ? (
                <p className="text-xs text-muted-foreground">
                  {label("No data", "لا توجد بيانات")}
                </p>
              ) : (
                <ResponsiveContainer width="100%" height={220}>
                  <AreaChart
                    data={stats.byMonth.map(([m, v]) => ({
                      month: m,
                      count: v,
                    }))}
                    onClick={(e: any) => {
            ...
            <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
              <h3 className="font-semibold mb-3">
                {label("Warranty Breakdown", "حالة الضمان")}
              </h3>
                ...
                <div className="flex items-center gap-4">
                  <ResponsiveContainer width="50%" height={160}>
                    <PieChart>
                      <Pie
                        data={[
                          {
                            name: label("Warranty", "ضمان"),
                            value: stats.warranty,
                          },
                          {
                            name: label("Out of Warranty", "خارج الضمان"),
                  ...
                  <div className="space-y-2 text-xs">
                    <div className="flex items-center gap-2">
                      ...
                      />
                      {label("Warranty", "ضمان")}:{" "}
                      <b className="tabular-nums">{stats.warranty}</b>
                    </div>
                    <div className="flex items-center gap-2">
                      <span
                        className="h-2.5 w-2.5 rounded-full"
                        style={{ background: CHART_COLORS[5] }}
                      />
                      {label("Out of Warranty", "خارج الضمان")}:{" "}
                      <b className="tabular-nums">{stats.outWarranty}</b>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
              <h3 className="font-semibold mb-3">
                {label("Final Status", "الحالة النهائية")}
              </h3>
              {finalArr.length === 0 ? (
                <p className="text-xs text-muted-foreground">
                  {label("No data", "لا توجد بيانات")}
            ...
            <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
              ...
              <div className="space-y-3">
                ...
                {stats.topClients.map(([k, v]) => (
                  <Bar key={k} label={k} count={v} max={maxClient} />
                ))}
              </div>
            </div>
            <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
              <h3 className="font-semibold mb-3">
                {label("Top Miner Models", "أهم موديلات الأجهزة")}
              </h3>
              <div className="space-y-3">
                ...
                {stats.topModels.map(([k, v]) => (
                  <Bar key={k} label={k} count={v} max={maxModel} />
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
            <h3 className="font-semibold mb-3">
              {label(
                "Serial Numbers by Status",
                "الأرقام المتسلسلة حسب الحالة",
              )}
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {[
                {
                  title: label("Ready for Dispatch", "جاهز للتسليم"),
                  list: stats.serialsReady,
                  tone: "text-emerald-600 dark:text-emerald-400",
                },
                {
                  title: label("In Repair", "قيد الصيانة"),
                  list: stats.serialsInRepair,
                  tone: "text-amber-600 dark:text-amber-400",
                },
                {
                  title: label("Awaiting Approval", "بانتظار الموافقة"),
                  list: stats.serialsAwaiting,
                  tone: "text-amber-600 dark:text-amber-400",
                },
                {
                  title: label("Dispatched", "تم التسليم"),
                  list: stats.serialsDispatched,
                  tone: "text-emerald-600 dark:text-emerald-400",
                },
                {
                  title: label("Warranty", "ضمان"),
                  list: stats.serialsWarranty,
                  tone: "text-primary",
                },
                {
                  title: label("Out of Warranty", "خارج الضمان"),
                  list: stats.serialsOutWarranty,
                  tone: "text-destructive",
                },
                {
                  title: label("Scrapped", "مهملة"),
                  list: stats.serialsScrapped,
                  tone: "text-destructive",
                },
              ].map((g) => (
                <div
