...
import BulkAddDialog, {
  type BulkAddRow,
} from "@/components/forms/BulkAddDialog";
...
const COLUMNS: {
  ...
  { key: "receiving_date", label: "Receiving Date", type: "date" },
  {
    ...
    label: "Customer Type",
    type: "select",
    options: ["General Customer", "Courier"],
  },
  ...
  { key: "receiving_location", label: "Receiving Location" },
  {
    ...
    type: "select",
    options: ["Warranty", "Out of 6"],
  },
  {
    key: "column_10",
    label: "Column 10",
    type: "select",
    options: ["Ticket assigned"],
  },
  { key: "work_order", label: "Work Order" },
  {
    ...
    type: "select",
    options: ["Approved", "Awaiting approved"],
  },
  {
    key: "repair_status",
    label: "Repair Status",
    type: "select",
    options: ["Went for repair"],
  },
  {
    key: "aging_status",
    label: "Aging Status",
    type: "select",
    options: ["Repair and went to test"],
  },
  {
    key: "quotation_sent",
  ...
  },
  {
    ...
    label: "Final Status",
    type: "select",
    options: ["Repair", "Scrape", "Return to customer"],
  },
...
const SelectButtonCell = ({
  ...
  // Compute display value: show placeholder hint if empty
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          className={cn(
            "h-7 w-full px-2 text-start text-xs text-foreground hover:bg-accent/40 focus:outline-none focus:ring-1 focus:ring-primary",
      ...
      <DropdownMenuContent
        align="start"
        className="min-w-40 bg-gray-600 dark:bg-popover border border-border"
      >
        <DropdownMenuItem
          className="text-xs text-foreground/60 focus:text-foreground/80 focus:bg-accent cursor-pointer"
          onClick={() => onChange("")}
        >
          Clear
        </DropdownMenuItem>
        {options.map((opt) => (
          <DropdownMenuItem
            key={opt}
            className="text-xs text-foreground focus:text-foreground focus:bg-accent cursor-pointer"
...
const MasterDataPage = () => {
  ...
  const getSyncState = (r: Row | null): "synced" | "pending" | "none" => {
    ...
    if (syncedKeys.ids.has(r.id) || (sn && syncedKeys.serials.has(sn)))
      return "synced";
    return "pending";
  };

  const fetchRecentDispatches = useCallback(async () => {
    const cutoff = new Date(Date.now() - DAYS_REPEAT * 86400000)
      .toISOString()
      .slice(0, 10);
  ...
  const createRowWith = async (
    ...
    if (key === "serial_number" && value) {
      ...
      if (match) {
        ...
        if (match.miner_model_and_type)
          payload.miner_model_and_type = match.miner_model_and_type;
      }
    }
    const { data, error } = await (supabase as any)
      .from("master_data")
  ...
  const commitDraft = async (index: number): Promise<Row | null> => {
    ...
    const hasValue = Object.values(draft).some(
    ...
    try {
      ...
      const payload: Row = {
        created_by: user?.id,
        receiving_date: cleanDraft.receiving_date || today(),
        ...cleanDraft,
      };
      if (draft.serial_number) {
        const match = serialLookup.get(normalizeSerial(draft.serial_number));
        if (match) {
          if (match.client_name && !payload.client_name)
            payload.client_name = match.client_name;
          if (match.miner_model_and_type && !payload.miner_model_and_type)
            payload.miner_model_and_type = match.miner_model_and_type;
        }
      }
      const { data, error } = await (supabase as any)
  ...
  const persistCell = (id: string, key: string, value: any) => {
    ...
    saveTimers.current[k] = setTimeout(async () => {
      const savedValue =
        key === "client_name" || key === "miner_model_and_type"
          ? normalizeText(value)
          : value;
  ...
  const updateCell = (id: string, key: string, value: any) => {
    ...
    setRows((prev) =>
      prev.map((r) => {
        ...
        if (
          next.serial_number ||
          next.psu_serial_number ||
          next.hash_board_serial_number
        ) {
          upsertReceiveItem({
            id: next.id,
            serialNumber: normalizeSerial(next.serial_number),
        ...
        if (key === "serial_number" && value) {
          ...
          if (match) {
            ...
            if (match.miner_model_and_type && !next.miner_model_and_type) {
              ...
              persistCell(
                id,
                "miner_model_and_type",
                match.miner_model_and_type,
              );
            }
          }
        }
  ...
  const pasteGridFrom = async (
    ...
    const grid = text
      ...
      .split("\n")
      .map((l) => l.split("\t"));
    while (grid.length && grid[grid.length - 1].every((x) => x === ""))
      grid.pop();
    ...
    for (let ri = 0; ri < grid.length; ri++) {
      ...
      for (let ci = 0; ci < cells.length; ci++) {
        ...
        if (isSerialKey(col.key) && raw) {
          ...
          if (!check.ok) {
            toast.error(`${col.key}: ${check.error}`);
            return true;
          }
          raw = check.value;
        }
        if (col.key === "client_name" || col.key === "miner_model_and_type")
  ...
  const bulkAddRows = async (items: BulkAddRow[]) => {
    ...
    for (const r of items.slice(0, 2000)) {
      ...
      if (!check.ok) {
        skipped.push(`${r.serial}: ${check.error}`);
        continue;
      }
      if (seen.has(check.value)) {
        skipped.push(`${check.value}: duplicate`);
        continue;
      }
    ...
    if (error) toast.error(sanitizeError(error));
      ...
      toast.success(
        `+${data?.length || 0} rows${skipped.length ? ` (skipped ${skipped.length})` : ""}`,
      );
      appendRows(data || []);
    }
  };
  ...
  const handleExport = async () => {
    ...
    XLSX.writeFile(
      wb,
      `master_data_${new Date().toISOString().slice(0, 10)}.xlsx`,
    );
  };
  ...
  const repeatRepair = useMemo(() => {
    const dispatchesBySerial = new Map<
      string,
  ...
  return (
    <div dir={dir} className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            size="sm"
            variant="outline"
            onClick={() => setPadCount((c) => c + ROW_PAD_STEP)}
          >
            + 1000 Rows
          </Button>
      ...
      <BulkAddDialog
        ...
        onSubmit={bulkAddRows}
        title="Bulk Add — Master Data"
      />
      ...
      >
        <div style={{ width: totalWidth, position: "relative" }}>
          ...
          >
            <div
              className="border-e border-border flex items-center justify-center font-semibold text-xs shrink-0"
              style={{ width: 48 }}
            >
              #
            </div>
            {COLUMNS.map((c) => (
              <div
                key={c.key}
                className="border-e border-border flex items-center px-2 font-semibold text-xs shrink-0"
              ...
              return (
                ...
                >
                  ...
                  {COLUMNS.map((c) => (
                    <div
                      key={c.key}
                      className="border-e border-b border-border shrink-0 p-0.5"
                      style={{ width: COL_WIDTH }}
                    >
                      {c.type === "select" ? (
                        <SelectButtonCell
                        ...
                        <div className="relative flex items-center gap-1">
                          <Input
                            ...
                            value={
                              r
                                ? (r[c.key] ?? "")
                                : (drafts[v.index]?.[c.key] ?? "")
                            }
                            data-row-idx={v.index}
                            data-col={c.key}
                            onPaste={async (e) => {
                              const text = e.clipboardData.getData("text");
                              const handled = await pasteGridFrom(
                                text,
                                v.index,
                                c.key,
                              );
                          ...
                          {c.key === "serial_number" &&
                            r &&
                            repeatRepair.has(r.id) && (
                              <span className="absolute right-1 top-1/2 -translate-y-1/2 text-[10px] bg-destructive text-destructive-foreground px-1 rounded">
                                2 repair
                              </span>
                            )}
                        </div>
                  ...
                  <div
                    className="border-e border-b border-border flex items-center justify-center shrink-0"
                    style={{ width: SYNC_WIDTH }}
                  >
                    {(() => {
                      ...
                      if (st === "synced")
                        return (
                          <span
                            className="inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded-full bg-emerald-500/15 text-emerald-600 dark:text-emerald-400"
                            title="Synced to Print queue"
                          >
                      ...
                      return (
                        <span
                          className="inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded-full bg-amber-500/15 text-amber-600 dark:text-amber-400"
                          title="Waiting to sync to Print queue"
                        >
                  ...
                  <div
                    className="border-b border-border flex items-center justify-center shrink-0"
                    style={{ width: 80 }}
                  >
                    {r && canDelete && (
                      <Button
